...
// ============================================
// ErrorBoundary — React 렌더링 오류를 잡아 빈 화면 대신 오류 메시지 표시
// ============================================
class TabErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="p-8 bg-red-50 border border-red-200 text-red-700 rounded-lg m-4">
          <p className="font-bold text-lg mb-2">탭 렌더링 오류 ({this.props.tabName})</p>
          <pre className="text-xs whitespace-pre-wrap bg-red-100 p-3 rounded overflow-auto max-h-60">
...
export default function FnFQ1_2026Dashboard() {
  ...
  const [cashFlowData, setCashFlowData] = useState([]);              // 현금흐름.csv 파싱 결과
  const [cashEntityNotes, setCashEntityNotes] = useState(() => loadFromStorage('fnf_q1_2026_cash_entity_notes') || {});
  const [cashEntityEditMode, setCashEntityEditMode] = useState(false);
  ...
  const [consolidatedCsvOverride, setConsolidatedCsvOverride] = useState({
    income: {
      ...
      '2026_1Q': {
        ...
        외환손익: 12831,
        영업외수익: 125955, 영업외비용: 10855,
        투자부동산처분손익: 104195,
        법인세비용차감전순이익: 268582, 법인세비용: 71026, 당기순이익: 197556,
      },
      '2026_1Q_Year': {
        매출액: 560898, 매출원가: 193139, 매출총이익: 367760,
        판매비와관리비: 214278, 급여: 20291, 퇴직급여: 1011, 인건비: 21302,
    ...
    balance: {
      ...
      '2026_1Q': {
        ...
        재고자산: 385593, 투자자산: 741072, 유무형자산: 656477,
        사용권자산: 175858, 기타자산: 137084, 자산총계: 2832461,
  ...
  // cashEntityNotes localStorage 저장
  React.useEffect(() => {
    try { localStorage.setItem('fnf_q1_2026_cash_entity_notes', JSON.stringify(cashEntityNotes)); } catch {}
  }, [cashEntityNotes]);
  ...
  const entityData = normalizeYearDataset({
    '매출액': {
      ...
      '2025_1Q': { 'OC(국내)': 215194, '중국': 258540, '홍콩': 20663, 'ST미국': 8443, '엔터테인먼트': 759, '기타': 2017 },
      '2025_1Q_Year': { 'OC(국내)': 215194, '중국': 258540, '홍콩': 20663, 'ST미국': 8443, '엔터테인먼트': 759, '기타': 2017 },
  ...
  // ─── 매출액/매출원가/매출총이익 2026_1Q 확정 수치 Override ─────────────────────
  // 연결운전자본(24~25).csv 기준: 연결 기여분 (내부거래 제거 후), 단위: 백만원
  // OC(국내) 223,917 / 중국 303,083 / 홍콩 25,832 / ST 7,469 / 기타(엔터) 597  합계 560,898
  if (entityData['매출액']) {
    entityData['매출액']['2026_1Q']      = { 'OC(국내)': 220684, '중국': 303083, '홍콩': 25832, '엔터테인먼트': 597, 'ST미국': 7469, '기타': 3233 };
    entityData['매출액']['2026_1Q_Year'] = { 'OC(국내)': 220684, '중국': 303083, '홍콩': 25832, '엔터테인먼트': 597, 'ST미국': 7469, '기타': 3233 };
  }
  if (entityData['매출원가']) {
    entityData['매출원가']['2026_1Q']      = { 'OC(국내)': 77282, '중국': 106271, '홍콩': 6497, '엔터테인먼트': 828, 'ST미국': 2261 };
    entityData['매출원가']['2026_1Q_Year'] = { 'OC(국내)': 77282, '중국': 106271, '홍콩': 6497, '엔터테인먼트': 828, 'ST미국': 2261 };
  }
  if (entityData['매출총이익']) {
    entityData['매출총이익']['2026_1Q']      = { 'OC(국내)': 146635, '중국': 196812, '홍콩': 19335, '엔터테인먼트': -231, 'ST미국': 5208 };
    entityData['매출총이익']['2026_1Q_Year'] = { 'OC(국내)': 146635, '중국': 196812, '홍콩': 19335, '엔터테인먼트': -231, 'ST미국': 5208 };
  }
  // ─────────────────────────────────────────────────────────────────────────
  ...
  if (entityBSData['2026_1Q']) {
    Object.assign(entityBSData['2026_1Q'], {
      ...
      재고자산:    { 'OC(국내)': 221710, 중국: 250497, 홍콩: 31242, ST미국: 10181, 기타:-128037 },
      ...
      유동자산:    { 'OC(국내)': 913463, 중국: 397247, 홍콩: 44968, ST미국: 18194, 기타:-200221 },
      ...
      자산총계:    { 'OC(국내)':2345491, 중국: 466687, 홍콩: 73005, ST미국: 91698, 기타:-144420 },
      부채총계:    { 'OC(국내)': 463125, 중국: 339268, 홍콩: 68236, ST미국: 37390, 기타: -56622 },
      자본총계:    { 'OC(국내)':1882366, 중국: 127419, 홍콩:  4768, ST미국: 54307, 기타: -87796 },
    ...
    const renderCard = (card, idx) => {
      ...
      if (card.isEPS) {
        ...
        return (
          <div key={idx} className="bg-white rounded-lg border border-zinc-200 shadow-sm p-4 hover:shadow-md transition-shadow duration-200">
            ...
            {/* 비교 기간 */}
            <div className="mb-1">
              <span className="text-xs text-zinc-400">{card.compareLabel || '전년'} {formatEPS(card.prevValue)}원</span>
              {epsDiff !== 0 && (
                <span className={`ml-1 font-bold text-xs ${epsDiff >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
            ...
            {card.epsAdj > 0 && (
              <div className="mt-1 -mx-4 -mb-4">
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 py-1.5 bg-indigo-50 hover:bg-indigo-100 transition-colors border-t border-indigo-100 text-xs font-medium text-indigo-500"
                >
                ...
                {epsAdjExpanded && (
                  <div className="bg-indigo-50 px-4 pb-3 pt-1">
                    <div className="flex items-baseline justify-between">
                      ...
                      {card.epsPrevAdj > 0 && (
                        <span className="text-xs text-indigo-400">전년 동분기 {formatEPS(card.epsPrevAdj)}원</span>
                      )}
                    </div>
                  </div>
                )}
      ...
      if (card.isPercent) {
        ...
        return (
          <div key={idx} className="bg-white rounded-lg border border-zinc-200 shadow-sm p-4 hover:shadow-md transition-shadow duration-200">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              {card.isYearEndMode && card.yoyValue != null ? (
                <div className="flex flex-col items-end gap-0.5">
                  <span className={`text-xs font-semibold px-1.5 py-0.5 rounded border ${isPositive ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'}`}>
                  ...
                  {(() => {
                    ...
                    return (
                      <span className={`text-xs font-semibold px-1.5 py-0.5 rounded border ${yoyPos ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'}`}>
            ...
            {/* 비교 기간 수치 */}
            <div className="mb-1">
              <span className="text-xs text-zinc-400">{card.compareLabel || '전년'} {card.prevValue.toFixed(1)}%</span>
              {diff !== 0 && (
                <span className={`ml-1 font-bold text-xs ${isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>
            ...
            {card.isYearEndMode && card.yoyValue != null && (() => {
              ...
              return (
                <>
                  <div className="mb-1">
                    <span className="text-xs text-zinc-400">{card.yoyLabel} {card.yoyValue.toFixed(1)}%</span>
                    {yoyDiff !== 0 && (
                      <span className={`ml-1 font-bold text-xs ${yoyDiff >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                        {yoyDiff >= 0 ? '+' : ''}{yoyDiff.toFixed(1)}%p
                      </span>
                    )}
                  </div>
                  <div className="text-xs text-indigo-500 leading-tight mt-0.5 italic">{memo}</div>
                </>
              );
            })()}
      ...
      return (
        <div key={idx} className="bg-white rounded-lg border border-zinc-200 shadow-sm p-4 hover:shadow-md transition-shadow duration-200 h-full">
          ...
          {/* 비교 기간 수치 및 차액 (한 줄) */}
          <div className="mb-2">
            <span className="text-xs text-zinc-400">{card.compareLabel || '전년'} {formattedPrev.number.replace('억원', '억')}억원</span>
            {diff !== 0 && (
              <span className={`ml-1 font-bold text-xs ${diff >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
              ...
              <div className="p-5 space-y-6">
                ...
                <div className="col-span-3">
                  <p className="text-[12px] font-semibold text-zinc-700 mb-1.5">
                    ① 연도별 실적추이 — 분기별 구성 <span className="font-normal text-zinc-400 text-xs">(단위: 억원)</span>
                  </p>
                  {(() => {
                    const qs = ['1Q','2Q','3Q','4Q'];
                    // 분기별 항목별 최고값 사전계산
                ...
                <div className="col-span-2">
                  <p className="text-[12px] font-semibold text-zinc-700 mb-1.5">
                    ② 비용구조 <span className="font-normal text-zinc-400 text-xs">(최근 5분기, %)</span>
                  </p>
                  {(() => {
                    const rows = [
                        ...
                        <table className="w-full text-xs border-collapse">
                          <thead>
                            <tr className="bg-zinc-50">
                              ...
                              <th className="text-right py-1.5 px-2 font-semibold text-zinc-400 border border-zinc-100">전동분기차<br/><span className="font-normal text-xs text-zinc-300">{yoyName ?? ''}</span></th>
                            </tr>
                          </thead>
                          <tbody>
              ...
              <div className="space-y-6">
                ...
                {false && <div className="bg-white rounded-xl border border-zinc-200 shadow-sm overflow-hidden">
                  <div className="px-4 py-3 border-b border-zinc-100 bg-rose-50/60">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-rose-800">자회사 현황 — ② 손상평가 모니터링 (엔터테인먼트 · ST미국)</h4>
                  </div>
                  <div className="p-4 space-y-5">
                    <p className="text-xs text-zinc-500 leading-relaxed">손상징후 발견 법인에 대한 25년 사업계획 대비 실적 달성 추이 모니터링. 사업계획 수치 입력 후 실적과 비교.</p>
                      ...
                      return (
                        <div key={entity} className="border-2 border-zinc-800 rounded-xl overflow-hidden bg-white shadow-sm">
                          ...
                          <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-100" style={{ background: `${color}12` }}>
                            ...
                            <span className="text-xs text-zinc-400">26.1Q 기준</span>
                          </div>
                          ...
                          <div className="grid grid-cols-5 divide-x divide-zinc-100">
                            ...
                            <div className="col-span-3 p-4 space-y-3">
                              ...
                              <div className="grid grid-cols-2 gap-3">
                                <div>
                                  <label className="text-xs font-semibold text-zinc-500 block mb-1">26.1Q 분기 계획 매출 (억원)</label>
                                  <input
                                    type="number"
                                    ...
                                    className="w-full border border-zinc-200 rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-200"
                                    placeholder="예: 150"
                                  />
                                </div>
                                <div>
                                  <label className="text-xs font-semibold text-zinc-500 block mb-1">26.1Q 분기 계획 영업이익 (억원)</label>
                                  <input
                                    type="number"
                              ...
                              {/* 계획 vs 실적 차트 */}
                              <div>
                                <p className="text-xs font-semibold text-zinc-500 mb-1.5">매출 · 영업손익 추이 및 계획 비교 (억원)</p>
                                  ...
                                  <ResponsiveContainer width="100%" height="100%">
                                    <ComposedChart data={planVsActualData} margin={{ top: 6, right: 10, left: -8, bottom: 0 }}>
                                      ...
                                      <Tooltip content={({ active, payload, label }) => {
                                        ...
                                        return (
                                          <div className="bg-white border border-zinc-200 rounded-lg shadow px-2.5 py-2 text-xs">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  {imp.plan2025Sales > 0 && actual26_1Q.매출액 != null && (
                                    <div className={`rounded-lg px-3 py-2 text-center border ${actual26_1Q.매출액 >= imp.plan2025Sales ? 'bg-emerald-50 border-emerald-100' : 'bg-rose-50 border-rose-100'}`}>
                                      <div className="text-xs text-zinc-500 mb-0.5">매출 달성률</div>
                                      ...
                                      <div className="text-xs text-zinc-400">{formatNumber(actual26_1Q.매출액)} / {formatNumber(imp.plan2025Sales)}억</div>
                                    </div>
                                  )}
                                  {imp.plan2025OpInc !== 0 && actual26_1Q.영업이익 != null && (
                                    <div className={`rounded-lg px-3 py-2 text-center border ${actual26_1Q.영업이익 >= imp.plan2025OpInc ? 'bg-emerald-50 border-emerald-100' : 'bg-rose-50 border-rose-100'}`}>
                                      <div className="text-xs text-zinc-500 mb-0.5">영업이익 달성률</div>
                                      ...
                                      <div className="text-xs text-zinc-400">{formatNumber(actual26_1Q.영업이익)} / {formatNumber(imp.plan2025OpInc)}억</div>
                                    </div>
                                  )}
                                </div>
                              )}
                            ...
                            <div className="col-span-2 p-4 flex flex-col gap-3">
                              ...
                              <div className="flex-1 flex flex-col">
                                ...
                                <textarea
                                  ...
                                  className="flex-1 w-full border border-emerald-100 bg-emerald-50/40 rounded-lg px-3 py-2 text-xs text-zinc-700 leading-relaxed resize-none focus:outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-200"
                              ...
                              {/* 모니터링 사항 */}
                              <div className="flex-1 flex flex-col">
                                <label className="text-xs font-bold text-rose-700 block mb-1.5 flex items-center gap-1">
                                ...
                                <textarea
                                  ...
                                  className="flex-1 w-full border border-rose-100 bg-rose-50/40 rounded-lg px-3 py-2 text-xs text-zinc-700 leading-relaxed resize-none focus:outline-none focus:border-rose-400 focus:ring-1 focus:ring-rose-200"
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              {aiEditMode && aiSaveStatus === 'saving' && (
                <span className="text-xs text-zinc-400 animate-pulse">저장 중...</span>
              )}
              ...
              return (
                <>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    {aiEditMode && (
                      <span className="ml-auto px-2 py-0.5 text-xs font-medium text-amber-400 bg-amber-400/20 rounded">편집 모드</span>
                    )}
                    {serverSaveStatus && (
                  ...
                  <div className="grid grid-cols-4 gap-2 mb-4 p-3 bg-white/5 rounded-lg border border-white/10">
                    <div className="text-center">
                      <div className="text-xs text-zinc-400 mb-0.5">영업이익률</div>
                      ...
                      <div className={`text-xs ${keyMetrics.opMargin.change >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {keyMetrics.opMargin.change >= 0 ? '+' : ''}{keyMetrics.opMargin.change.toFixed(1)}%p
                      </div>
                    </div>
                    <div className="text-center border-l border-white/10">
                      <div className="text-xs text-zinc-400 mb-0.5">순이익률</div>
                      ...
                      <div className={`text-xs ${keyMetrics.netMargin.change >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {keyMetrics.netMargin.change >= 0 ? '+' : ''}{keyMetrics.netMargin.change.toFixed(1)}%p
                      </div>
                    </div>
                    <div className="text-center border-l border-white/10">
                      <div className="text-xs text-zinc-400 mb-0.5">부채비율</div>
                      ...
                      <div className="text-xs text-emerald-400">{keyMetrics.debtRatio.status}</div>
                    </div>
                    <div className="text-center border-l border-white/10">
                      <div className="text-xs text-zinc-400 mb-0.5">ROE</div>
                      <div className={`text-sm font-bold ${keyMetrics.roe.change >= 0 ? 'text-emerald-400' : 'text-violet-400'}`}>
                        {keyMetrics.roe.curr.toFixed(1)}%
                      </div>
                      <div className={`text-xs ${keyMetrics.roe.change >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                              ...
                              <div className="flex-1">
                                {aiEditMode ? (
                                  <div className="space-y-2">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                      <div>
                                        <label className="text-xs text-zinc-400">현재</label>
                                        <input
                                          type="text"
                                      ...
                                      <div>
                                        <label className="text-xs text-zinc-400">목표</label>
                                        <input
                                          type="text"
                                          value={target.target}
                                    ...
                                    <div>
                                      <label className="text-xs text-emerald-400">예상 효과</label>
                                      <textarea
                                        value={target.impact}
                                    ...
                                    <div>
                                      <label className="text-xs text-blue-400">실행 방안</label>
                                      <textarea
                                        value={target.method}
                                    ...
                                    <div>
                                      <label className="text-xs text-amber-400">목표 근거 (선택)</label>
                                      <textarea
                  ...
                  <div className="mt-4 pt-3 border-t border-white/10 flex items-center justify-between">
                    <span className="text-xs text-zinc-500">AI 분석은 참고용이며 투자 조언이 아닙니다</span>
                    <span className="text-xs text-zinc-500">{selectedPeriod ? `${selectedPeriod.split('_')[0]}년 ${selectedPeriod.split('_')[1]} 기준` : '데이터 기준'}</span>
                  </div>
                </>
              );
              } catch (error) {
  ...
  const renderIncomeTab = () => {
    ...
    const getBaseEntityBreakdown = (accountKey, period) => {
      ...
      // ── 법인별 구성 하드코딩 오버라이드 (2026_1Q) ──
      const rp = period.replace(/_Year$/, '');
      if (rp === '2026_1Q') {
        // 선물환손익: OC(국내) 귀속분 없음 (연결조정 항목) → 전체 0
        if (accountKey === '선물환손익') return { 'OC(국내)': 0 };
      ...
      <div className="space-y-4">
        ...
        <div className="w-full xl:w-[45%] xl:min-w-[420px] flex-shrink-0 space-y-3">
          ...
          {(() => {
            ...
            // 고정비(인건비+감가상각비+기타판관비) / 변동비(광고선전비+수수료)
            const fixedC    = laborC + deprC + otherC;
            const variableC = adC + commC;
            const fixedP    = laborP + deprP + otherP;
            const variableP = adP + commP;
            ...
            const cogsRateP = cogsP / salesP;
            const cogsRateC = salesC ? cogsC / salesC : 0;
            const sgaRateP  = sgaP / salesP;
            const sgaRateC  = salesC ? sgaC / salesC : 0;
            const gmP = 1 - cogsRateP;
            ...
            // 영업 레버리지 (DOL = 매출총이익 / 영업이익)
            const grossProfitC = salesC - cogsC;
            const dol = opC ? grossProfitC / opC : 0;

            // 시뮬레이션: 고정비 불변, 변동비(광고+수수료) 매출 비례, GM% 유지
            const simGrowths = [-0.05, 0, 0.03, 0.05, 0.10, 0.15, 0.20];
            const simResults = simGrowths.map(gr => {
              const ns  = salesC    * (1 + gr);
              const nc  = cogsC     * (1 + gr);
              const nv  = variableC * (1 + gr);
              const nf  = fixedC;
              const nop = ns - nc - nf - nv;
              return { gr, ns, nop, nom: ns ? nop / ns : 0 };
            });
              ...
              <div className="bg-white rounded-lg border border-zinc-200 shadow-sm p-4 text-xs">
                ...
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-[10px] text-zinc-400 mt-0.5">대표이사 보고용 · {prevLbl} → {currLbl}</p>
                  </div>
                  {/* 헤더 KPI 뱃지 */}
                  <div className="flex gap-1.5">
                    <div className="text-center bg-zinc-50 border border-zinc-100 rounded px-2 py-1">
                      <div className="text-[9px] text-zinc-400">OM</div>
                      <div className="text-xs font-bold text-zinc-800">{pct1(omC)}</div>
                      <div className={`text-[10px] font-semibold tabular-nums ${cOM}`}>{pp1(deltaOM)}</div>
                    </div>
                    <div className="text-center bg-zinc-50 border border-zinc-100 rounded px-2 py-1">
                      <div className="text-[9px] text-zinc-400">GM</div>
                      <div className="text-xs font-bold text-zinc-800">{pct1(gmC)}</div>
                      <div className={`text-[10px] font-semibold tabular-nums ${cGM}`}>{pp1(deltaGM)}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[#1e3a5f]/5 border border-[#1e3a5f]/15 rounded-lg p-3 mb-3">
                  ...
                  <div className="grid grid-cols-4 gap-1 mb-2.5">
                    {[
                      ...
                      { label: '원가율', v: pp1(-deltaGM>0?deltaGM:-deltaGM)+(deltaGM>=0?' ↓':' ↑'), c: cGM },
                      { label: '판관비율', v: pp1(-deltaSGA), c: -deltaSGA>=0?'text-emerald-600':'text-rose-600' },
                      { label: '영업이익률', v: pp1(deltaOM), c: cOM },
                    ].map(({label,v,c}) => (
                  ...
                  <div className="space-y-1.5 text-[11px] text-zinc-700 leading-relaxed">
                    <p>
                      <span className="font-semibold text-zinc-900">{currLbl} 매출액 {formatNumber(salesC)}백만원</span>으로 전년 동기 대비{' '}
                      <span className={`font-semibold ${s>=0?'text-emerald-700':'text-rose-700'}`}>{pp(s)}</span> 성장.
                      매출원가는 <span className="font-semibold">{pp(c)}</span> 증가에 그쳐 원가율이{' '}
                      <span className={`font-semibold ${cGM}`}>{pct1(cogsRateP)} → {pct1(cogsRateC)}</span>로{' '}
                      <span className={`font-semibold ${cGM}`}>{pp(deltaGM)}</span> 개선. 이는 매출 성장(+{(s*100).toFixed(1)}%)이 원가 증가(+{(c*100).toFixed(1)}%)를{' '}
                      <span className="font-semibold text-zinc-800">{((s-c)*100).toFixed(1)}%p 상회</span>한 결과임.
                    </p>
                    <p>
                      판관비는 <span className="font-semibold">{pp(g)}</span> 증가로 매출 성장률을{' '}
                      <span className={`font-semibold ${g<=s?'text-emerald-700':'text-rose-700'}`}>{((s-g)*100).toFixed(1)}%p 하회</span>,
                      판관비율 <span className={`font-semibold ${cSGA}`}>{pct1(sgaRateP)} → {pct1(sgaRateC)}</span>({pp(deltaSGA)}) 개선.
                      특히 수수료·광고선전비 등 변동비는{' '}
                      <span className={`font-semibold ${varG<=s?'text-emerald-700':'text-rose-700'}`}>{pp(varG)}</span> 증가로
                      매출 성장 대비 <span className="font-semibold text-zinc-800">{((s-varG)*100).toFixed(1)}%p 억제</span>되어
                      판관비 레버리지 효과가 발현됨.
                    </p>
                    <p>
                      결과적으로 영업이익률 <span className={`font-semibold ${cOM}`}>{pct1(omP)} → {pct1(omC)}</span>({pp(deltaOM)}) 상승.
                      이 중 원가 관리 기여 <span className={`font-semibold ${cGM}`}>{pp(deltaGM)}</span>,
                      판관비 레버리지 기여 <span className={`font-semibold ${-deltaSGA>=0?'text-emerald-700':'text-rose-700'}`}>{pp(-deltaSGA)}</span>.
                      영업 레버리지(DOL) <span className="font-semibold text-zinc-800">{dol.toFixed(1)}배</span>로,
                      추가 매출 성장 시 이익률 레버리지 효과 지속 가능.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="border border-zinc-100 rounded-md p-2.5">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className={`text-sm font-bold tabular-nums ${cGM}`}>{pp(deltaGM)}</span>
                    </div>
                  ...
                  <div className="border border-zinc-100 rounded-md p-2.5">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className={`text-sm font-bold tabular-nums ${cOM}`}>{pp(deltaOM)}</span>
                    </div>
                    <div className="space-y-0.5">
                      ...
                      <div className="border-t border-zinc-100 mt-1 pt-1 space-y-0.5">
                        ...
                        <Row label={`${prevLbl} OM`} value={pct1(omP)} sub />
                        <Row label={`${currLbl} OM`} value={pct1(omC)} sub />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border border-zinc-100 rounded-md p-2.5">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-semibold text-zinc-700">③ 판관비 고정비·변동비 분석</span>
                      <span className="text-[10px] text-zinc-400">{prevLbl} / {currLbl}</span>
                    </div>
                    <table className="w-full" style={{fontSize:'10px'}}>
                      <thead>
                        <tr className="border-b border-zinc-100">
                          <th className="text-left py-1 font-medium text-zinc-500 w-[80px]">항목</th>
                          <th className="text-center py-1 font-medium text-zinc-400 w-[36px]">성격</th>
                          <th className="text-right py-1 font-medium text-zinc-400">금액</th>
                          <th className="text-right py-1 font-medium text-zinc-400 w-[38px]">매출대비</th>
                          <th className="text-right py-1 font-semibold text-zinc-600">금액</th>
                          <th className="text-right py-1 font-semibold text-zinc-500 w-[38px]">매출대비</th>
                        </tr>
                      </thead>
                      <tbody>
                        {[
                          { label:'인건비',     P:laborP, C:laborC, type:'고정', tc:'bg-blue-50 text-blue-600' },
                          { label:'감가상각비', P:deprP,  C:deprC,  type:'고정', tc:'bg-blue-50 text-blue-600' },
                          { label:'기타판관비', P:otherP, C:otherC, type:'준고정', tc:'bg-purple-50 text-purple-600' },
                          { label:'광고선전비', P:adP,    C:adC,    type:'변동', tc:'bg-orange-50 text-orange-600' },
                          { label:'수수료',     P:commP,  C:commC,  type:'변동', tc:'bg-orange-50 text-orange-600' },
                        ].map(({label, P, C, type, tc}) => (
                          <tr key={label} className="border-b border-zinc-50 hover:bg-zinc-50/40">
                            <td className="py-1 text-zinc-600">{label}</td>
                            <td className="py-1 text-center">
                              <span className={`text-[9px] px-1 py-0.5 rounded font-medium ${tc}`}>{type}</span>
                            </td>
                            <td className="py-1 text-right text-zinc-400 tabular-nums">{formatNumber(P)}</td>
                            <td className="py-1 text-right text-zinc-400 tabular-nums">{salesP ? `${(P/salesP*100).toFixed(1)}%` : '—'}</td>
                            <td className="py-1 text-right text-zinc-700 tabular-nums font-medium">{formatNumber(C)}</td>
                            <td className="py-1 text-right text-zinc-500 tabular-nums">{salesC ? `${(C/salesC*100).toFixed(1)}%` : '—'}</td>
                          </tr>
                        ))}
                        <tr className="border-t border-zinc-200 bg-blue-50/40">
                          <td className="py-1 font-semibold text-blue-700">고정비 계</td>
                          <td></td>
                          <td className="py-1 text-right text-zinc-500 tabular-nums">{formatNumber(fixedP)}</td>
                          <td className="py-1 text-right text-zinc-400 tabular-nums">{salesP ? `${(fixedP/salesP*100).toFixed(1)}%` : '—'}</td>
                          <td className="py-1 text-right text-blue-800 tabular-nums font-bold">{formatNumber(fixedC)}</td>
                          <td className="py-1 text-right text-blue-700 tabular-nums font-semibold">{salesC ? `${(fixedC/salesC*100).toFixed(1)}%` : '—'}</td>
                        </tr>
                        <tr className="bg-orange-50/40">
                          <td className="py-1 font-semibold text-orange-700">변동비 계</td>
                          <td></td>
                          <td className="py-1 text-right text-zinc-500 tabular-nums">{formatNumber(variableP)}</td>
                          <td className="py-1 text-right text-zinc-400 tabular-nums">{salesP ? `${(variableP/salesP*100).toFixed(1)}%` : '—'}</td>
                          <td className="py-1 text-right text-orange-800 tabular-nums font-bold">{formatNumber(variableC)}</td>
                          <td className="py-1 text-right text-orange-700 tabular-nums font-semibold">{salesC ? `${(variableC/salesC*100).toFixed(1)}%` : '—'}</td>
                        </tr>
                      </tbody>
                    </table>
                    <div className="mt-2 pt-1.5 border-t border-zinc-100 flex items-center justify-between">
                      <span className="text-[10px] text-zinc-500">영업 레버리지 (DOL = 매출총이익 ÷ 영업이익)</span>
                      <span className="text-xs font-bold text-zinc-800">{dol.toFixed(2)}×</span>
                    </div>
                    <p className="text-[9px] text-zinc-400 mt-1 leading-relaxed">
                      DOL {dol.toFixed(1)}배 → 매출 1% 증가 시 영업이익 약 {dol.toFixed(1)}% 증가 효과.
                      고정비 비중({salesC?`${(fixedC/salesC*100).toFixed(1)}%`:'—'}) 높을수록 레버리지 확대.
                    </p>
                  </div>
                  ...
                  <div className="border border-zinc-100 rounded-md p-2.5">
                    <div className="mb-2">
                      <span className="text-xs font-semibold text-zinc-700">④ 영업이익률 개선 시뮬레이션</span>
                      <p className="text-[10px] text-zinc-400 mt-0.5 leading-relaxed">
                        가정: GM% 유지 · 고정비 불변({formatNumber(fixedC)}백만원) · 변동비 매출 비례
                      </p>
                    </div>
                    <table className="w-full" style={{fontSize:'10px'}}>
                      <thead>
                        <tr className="border-b border-zinc-100">
                          <th className="text-left py-1 font-medium text-zinc-500">매출 성장</th>
                          <th className="text-right py-1 font-medium text-zinc-400">매출액</th>
                          ...
                          <th className="text-right py-1 font-medium text-zinc-400">vs BASE</th>
                        </tr>
                      </thead>
                      <tbody>
                        {simResults.map(({gr, ns, nop, nom}) => {
                          ...
                          const d = nom - omC;
                          return (
                            <tr key={gr} className={`border-b border-zinc-50 ${isBase ? 'bg-zinc-100/60 font-semibold' : 'hover:bg-zinc-50/40'}`}>
                              <td className={`py-1 tabular-nums ${gr < 0 ? 'text-rose-600' : gr === 0 ? 'text-zinc-600' : 'text-emerald-600'}`}>
                                {gr === 0 ? `±0% BASE` : `${gr>0?'+':''}${(gr*100).toFixed(0)}%`}
                              </td>
                              <td className="py-1 text-right text-zinc-500 tabular-nums">{formatNumber(Math.round(ns))}</td>
                              <td className="py-1 text-right text-zinc-500 tabular-nums">{formatNumber(Math.round(nop))}</td>
                              <td className={`py-1 text-right tabular-nums font-bold ${nom > omC ? 'text-emerald-600' : nom < omC ? 'text-rose-600' : 'text-zinc-700'}`}>
                                {pct1(nom)}
                              </td>
                              <td className={`py-1 text-right tabular-nums ${isBase ? 'text-zinc-400' : d>=0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                                {isBase ? '—' : pp1(d)}
                              </td>
                            </tr>
                          );
                    ...
                    <p className="text-[9px] text-zinc-400 mt-1.5 leading-relaxed">
                      ※ 구조적 비용 절감 없이 외형 성장만으로의 이익률 변화 추정.
                      추가 판관비 절감 or 원가율 개선 병행 시 실제 OM은 시나리오보다 높아질 수 있음.
                    </p>
                  </div>
                </div>
              </div>
          ...
          return (
            <div className="bg-white rounded-lg border border-zinc-200 shadow-sm p-4 text-xs">
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex gap-1.5">
                  {[{label:'GM',val:pct1(gmC),delta:pp1(deltaGM),c:cGM},{label:'OM',val:pct1(omC),delta:pp1(deltaOM),c:cOM}].map(({label,val,delta,c})=>(
                    <div key={label} className="text-center bg-zinc-50 border border-zinc-100 rounded px-2 py-1">
                      <div className="text-[9px] text-zinc-400">{label}</div>
                      <div className="text-xs font-bold text-zinc-800">{val}</div>
                      <div className={`text-[10px] font-semibold tabular-nums ${c}`}>{delta}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-[#1e3a5f]/5 border border-[#1e3a5f]/15 rounded-lg p-3 mb-3">
                ...
                <div className="grid grid-cols-4 gap-1 mb-2.5">
                  {[
                    {label:'매출 성장',v:pp1(s),c:s>=0?'text-emerald-600':'text-rose-600'},
                    {label:'원가율 변동',v:pp1(deltaGM)+(deltaGM>=0?' ↓':' ↑'),c:cGM},
                    {label:'판관비율 변동',v:pp1(-deltaSGA),c:-deltaSGA>=0?'text-emerald-600':'text-rose-600'},
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[11px] text-zinc-700 leading-relaxed">
                  <p>
                    <span className="font-semibold text-zinc-900">{currLbl} 매출 {formatNumber(salesC)}백만원</span> ({pp(s)}) 성장.
                    원가율 <span className={`font-semibold ${cGM}`}>{pct1(cogsRateP)}→{pct1(cogsRateC)}</span>({pp(deltaGM)}),
                    판관비율 <span className={`font-semibold ${cSGA}`}>{pct1(sgaRateP)}→{pct1(sgaRateC)}</span>({pp(deltaSGA)}) 개선.
                    영업이익률 <span className={`font-semibold ${cOM}`}>{pct1(omP)}→{pct1(omC)}</span>({pp(deltaOM)}) 달성.
                    OM 기여: 원가관리 <span className={`font-semibold ${cGM}`}>{pp(deltaGM)}</span> + 판관비 레버리지 <span className={`font-semibold ${-deltaSGA>=0?'text-emerald-700':'text-rose-700'}`}>{pp(-deltaSGA)}</span>.
                  </p>
                  <p>
                    매출총이익 <span className={`font-semibold ${gpGrowth>=0?'text-emerald-700':'text-rose-700'}`}>{pp(gpGrowth)}</span> 성장 분해:
                    외형 성장 기여 <span className="font-semibold text-zinc-800">{pp(s)}</span>
                    + 마진 개선 기여 <span className={`font-semibold ${cGM}`}>{pp(marginEffect)}</span> (= ΔGM{pp(deltaGM)}÷GM₀{pct1(gmP)})
                    + 교차항 <span className="text-zinc-600">{pp(crossEffect)}</span>.
                    변동비 증가율({pct(varG)})이 매출 대비 <span className={`font-semibold ${varG<=s?'text-emerald-700':'text-rose-700'}`}>{((s-varG)*100).toFixed(1)}%p 억제</span>,
                    DOL <span className="font-semibold text-zinc-800">{dol.toFixed(1)}배</span> 레버리지 효과 발현.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 items-start">
                ...
                <div className="border border-zinc-100 rounded-md p-2.5">
                  ...
                  <div className="space-y-0.5">
                    <Row label="판관비 증가율 (g)" value={pct(g)} />
                    <Row label="차이 (g−s)" value={`${((g-s)*100).toFixed(2)}%p`} valueColor={g<=s?'text-emerald-600':'text-rose-600'} />
                    <Row label={`판관비율₀ (${prevLbl})`} value={pct1(sgaRateP)} />
                    <Row label={<span>Δ판관비율 = 판관비율₀×(g−s)÷(1+s)<br/><span className="text-[9px] text-zinc-400 tabular-nums">= {pct1(sgaRateP)}×{((g-s)*100).toFixed(2)}%p÷{(1+s).toFixed(3)}</span></span>} value={pp(deltaSGA)} valueColor={cSGA} />
                    <div className="border-t border-zinc-100 mt-1 pt-1 space-y-0.5">
                      <Row label="ⓐ GM 개선 효과" value={pp(deltaGM)} valueColor={cGM} sub />
                      <Row label="ⓑ 판관비율 레버리지" value={pp(-deltaSGA)} valueColor={-deltaSGA>=0?'text-emerald-600':'text-rose-600'} sub />
                      <div className="flex items-center justify-between border-t border-zinc-100 pt-1 mt-0.5">
                        <span className="text-xs font-semibold text-zinc-700">ΔOM = ⓐ+ⓑ</span>
                        <span className={`text-xs font-bold tabular-nums ${cOM}`}>{pp(deltaOM)}</span>
                      </div>
                ...
                <div className="border border-zinc-100 rounded-md p-2.5">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-semibold text-zinc-700">③ 판관비 고정·변동 분석</span>
                  </div>
                  <table className="w-full" style={{fontSize:'10px'}}>
                    <thead>
                      <tr className="border-b border-zinc-100">
                        <th className="text-left py-1 font-medium text-zinc-500">항목</th>
                        <th className="text-center py-1 font-medium text-zinc-400 w-[30px]">성격</th>
                        <th className="text-right py-1 font-semibold text-zinc-600">{currLbl}</th>
                        <th className="text-right py-1 font-medium text-zinc-400 w-[40px]">매출대비</th>
                      </tr>
                    </thead>
                    <tbody>
                      {[
                        {label:'인건비',C:laborC,P:laborP,type:'고정',tc:'bg-blue-50 text-blue-600'},
                        {label:'감가상각비',C:deprC,P:deprP,type:'고정',tc:'bg-blue-50 text-blue-600'},
                        {label:'기타판관비',C:otherC,P:otherP,type:'준고정',tc:'bg-purple-50 text-purple-600'},
                        {label:'광고선전비',C:adC,P:adP,type:'변동',tc:'bg-orange-50 text-orange-600'},
                        {label:'수수료',C:commC,P:commP,type:'변동',tc:'bg-orange-50 text-orange-600'},
                      ].map(({label,C,type,tc})=>(
                  ...
                  <div className="mt-2 pt-1.5 border-t border-zinc-100 flex items-center justify-between">
                    <span className="text-[10px] text-zinc-500">DOL = 매출총이익 ÷ 영업이익</span>
                    <span className="text-xs font-bold text-zinc-800">{dol.toFixed(2)}×</span>
                  </div>
                  <p className="text-[9px] text-zinc-400 mt-1">매출 1%↑ → 영업이익 약 {dol.toFixed(1)}% 증가 효과. 고정비({salesC?`${(fixedC/salesC*100).toFixed(1)}%`:'—'}) 비중↑ → 레버리지 확대.</p>
                </div>
                ...
                <div className="border border-zinc-100 rounded-md p-2.5">
                  ...
                  <p className="text-[9px] text-zinc-400 mt-1.5">※ 외형 성장만으로의 OM 변화 추정. 추가 비용 절감 시 실제 OM은 시나리오보다 높아질 수 있음.</p>
                </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-[11px]" style={{tableLayout:'fixed', minWidth:'760px'}}>
                      ...
                      <tbody>
                        ...
                        <tr className="border-b border-zinc-200 hover:bg-zinc-50">
                          <td className="px-2 py-1.5 text-zinc-500 text-[10px] border-r border-zinc-200 align-top">중국제외자회사<br/><span className="text-zinc-400">(홍콩,엔터, ST미국)</span></td>
                          <td colSpan={2} className="px-2 py-1.5 tabular-nums text-zinc-400 text-[10px] leading-relaxed border-r border-zinc-200">
                            홍콩 40 / 엔터 20<br/>ST미국 15
                          </td>
                          <td className="text-right px-2 py-1.5 tabular-nums text-zinc-600 border-r border-zinc-200">75</td>
                          <td className="text-right px-2 py-1.5 border-r border-zinc-200"></td>
                          <td colSpan={2} className="px-2 py-1.5 tabular-nums text-zinc-400 text-[10px] leading-relaxed border-r border-zinc-200">
                            홍콩 42 / 엔터 20<br/>ST미국 5
                          </td>
                          <td className="text-right px-2 py-1.5 tabular-nums text-zinc-600 border-r border-zinc-100">68</td>
                          <td className="text-right px-2 py-1.5"></td>
                        </tr>
                    ...
                    <table className="w-full text-[11px]" style={{tableLayout:'fixed', minWidth:'640px'}}>
                      ...
                      <tbody>
                        ...
                        <tr className="border-b border-zinc-200 hover:bg-zinc-50">
                          <td className="px-2 py-1.5 text-zinc-500 text-[10px] border-r border-zinc-200 align-top">중국제외자회사<br/><span className="text-zinc-400">(홍콩,엔터, ST미국)</span></td>
                          <td colSpan={2} className="px-2 py-1.5 tabular-nums text-zinc-400 text-[10px] leading-relaxed border-r border-zinc-200">
                            홍콩 201 / 엔터 237<br/>ST미국 213
                          </td>
                          <td className="text-right px-2 py-1.5 tabular-nums text-zinc-600 border-r border-zinc-200">652</td>
                          <td colSpan={2} className="px-2 py-1.5 tabular-nums text-zinc-400 text-[10px] leading-relaxed border-r border-zinc-200">
                            홍콩 165 / 엔터 299<br/>ST미국 524
                          </td>
                          <td className="text-right px-2 py-1.5 tabular-nums text-zinc-600">988</td>
                        </tr>
                        {/* 연결 합계 */}
                ...
                <div className="bg-white rounded-lg border border-zinc-200 shadow-sm overflow-hidden">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-[12px]">
                      <thead>
                        <tr className="bg-zinc-100 border-b border-zinc-200 text-zinc-600">
                          <th className="text-left px-3 py-2 font-semibold border-r border-zinc-200 w-36">항목</th>
                          <th className="text-right px-3 py-2 font-semibold border-r border-zinc-200 bg-zinc-200/60 text-zinc-500">24.4Q</th>
                          <th className="text-right px-3 py-2 font-semibold border-r border-zinc-200 bg-blue-50/60 text-blue-700">25.1Q</th>
                          <th className="text-right px-3 py-2 font-semibold border-r border-zinc-200 bg-amber-50/60 text-amber-700">25.4Q</th>
                      ...
                      <tbody>
                        <tr className="border-b border-zinc-100 bg-zinc-50/60">
                          ...
                          <td className="text-right px-3 py-1.5 tabular-nums text-zinc-500 font-semibold border-r border-zinc-200">6.32%</td>
                          <td className="text-right px-3 py-1.5 tabular-nums text-blue-700 font-semibold border-r border-zinc-200">6.32%</td>
                          <td className="text-right px-3 py-1.5 tabular-nums text-amber-700 font-semibold border-r border-zinc-200">6.32%</td>
                        ...
                        <tr className="border-b border-zinc-100 hover:bg-zinc-50">
                          ...
                          <td className="text-right px-3 py-1.5 tabular-nums text-zinc-500 border-r border-zinc-200">2024-09-30</td>
                          <td className="text-right px-3 py-1.5 tabular-nums text-zinc-500 border-r border-zinc-200">2025-01-16</td>
                          <td className="text-right px-3 py-1.5 tabular-nums text-zinc-500 border-r border-zinc-200">2025-12-15</td>
                        ...
                        <tr className="border-b border-zinc-100 hover:bg-zinc-50">
                          ...
                          <td className="text-right px-3 py-1.5 tabular-nums text-zinc-700 border-r border-zinc-200">1,846,200,794</td>
                          <td className="text-right px-3 py-1.5 tabular-nums text-zinc-700 border-r border-zinc-200">1,724,078,728</td>
                          <td className="text-right px-3 py-1.5 tabular-nums text-zinc-700 border-r border-zinc-200">381,547,774</td>
                        ...
                        <tr className="border-b border-zinc-100 bg-zinc-50/40 hover:bg-zinc-50">
                          ...
                          <td className="text-right px-3 py-1.5 tabular-nums text-zinc-700 border-r border-zinc-200">2,523,521 주</td>
                        ...
                        <tr className="border-b border-zinc-100 hover:bg-zinc-50">
                          ...
                          <td className="text-right px-3 py-1.5 tabular-nums text-zinc-700 border-r border-zinc-200">40,323,089,707</td>
                          <td className="text-right px-3 py-1.5 tabular-nums text-zinc-700 border-r border-zinc-200">12,631,909,131</td>
                          <td className="text-right px-3 py-1.5 tabular-nums text-zinc-700 border-r border-zinc-200">4,536,764,239</td>
                        ...
                        <tr className="border-b border-zinc-200 hover:bg-zinc-50">
                          ...
                          <td className="text-right px-3 py-1.5 tabular-nums text-zinc-700 border-r border-zinc-200">2,546,721,455</td>
                          <td className="text-right px-3 py-1.5 tabular-nums text-zinc-700 border-r border-zinc-200">797,804,787</td>
                          <td className="text-right px-3 py-1.5 tabular-nums text-zinc-700 border-r border-zinc-200">286,532,478</td>
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="w-full lg:w-[45%] lg:min-w-[420px] flex-shrink-0 space-y-3">
                      ...
                      {processedRows.map((pr, i) => {
                        ...
                        return (
                          <tr key={i} className="border-b border-zinc-100">
                            <td className="px-3 py-1 pl-5 text-zinc-700 whitespace-nowrap">{pr.label}</td>
            ...
            {selectedBSAccount === '자본총계' && (
              ...
              {/* 자본 상세 테이블 */}
              {/* 자본 상세 테이블: 25년말 vs 26.1Q말 (2026_BS.csv 기반) */}
        ...
        <div className="bg-white rounded-lg border border-zinc-200 shadow-sm overflow-hidden">
          ...
          {/* 우실: 분기별 추이 */}
          <div className="w-full lg:w-[45%] lg:min-w-[420px] flex-shrink-0">
        {/* 분기별 추이 그래프 */}
        {balanceItems.find(i => i.key === selectedBSAccount)?.selectable && quarterlyEntityData[selectedBSAccount] && (
          <div className="bg-white rounded-lg border border-zinc-200 shadow-sm p-4">
            <div className="flex items-center justify-between mb-3">
        ...
        <div className="mt-4 bg-white rounded-lg border border-zinc-200 shadow-sm p-4">
          ...
          {bsEntitySubTab === '연결' && (() => {
            const bsP = bsCurrentPeriod;
            const bsRow = balanceSheetData[bsP] || {};

            // ===== 영업운전자본 (NWC = 매출채권 + 재고자산 - 매입채무) =====
            const qSalesM = incomeStatementData[bsCurrentPeriod]?.매출액 || 0;
            const qCogsM = incomeStatementData[bsCurrentPeriod]?.매출원가 || 0;
  ...
  const renderEntityStatementsTab = ({ forceEntity = null, forceMode = 'all' } = {}) => {
    ...
    const getISRaw = (account, period) => {
      ...
      if (selectedEntityKey === 'OC(국내)') {
        // 선물환손익: 연결조정 항목으로 OC(국내) 귀속분 없음 (26.1Q만 0 처리)
      ...
      if (csvBs) {
        ...
        if (account === '기타부채') {
          for (const ek of entityCandidates) {
            ...
            // 금융부채는 표시 항목에서 제거 → 기타부채에 흡수
            return Math.round(Number(total) - payables - accrued - deposit - borr - lease);
          }
        }
      }
      // ────────────────────────────────────────────────────────────────────
